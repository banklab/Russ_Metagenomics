...
setwd("/data/projects/p898_Deer_RAS_metagenomics/04_Deer/LONG_READS/CoverM")
diversity_df <- data.frame(fread("DEER_v2_CoverM.csv", header=T, stringsAsFactors = F))

diversity_df$Deer <- as.numeric(gsub("_.*","",diversity_df$Sample))
...
snp_file_list <- list.files(pattern="_LR_InStrain_SNVs_format.csv")
...
for(i in 1:length(snp_file_list)){
 ...
 setwd("/data/projects/p898_Deer_RAS_metagenomics/04_Deer/LONG_READS/11_InStrain/FORMAT")
  ...
  setwd("/data/projects/p898_Deer_RAS_metagenomics/04_Deer/LONG_READS/Diversity_all")
  write.csv(div_by_site, paste0(DEER,"_",ENV, "_LR28SR70_Diversity_by_site.csv"), row.names = F)
  
  diversity_df_for_sample <- diversity_df[diversity_df$Deer==DEER & diversity_df$Env==ENV,]
  
  
  write.csv(diversity_df_for_sample, paste0(DEER,"_",ENV, "_LR28SR70_Diversity_by_sample.csv"), row.names = F)
  
}


Diversity_files <- list.files(pattern="LR28SR70_Diversity_by_sample.csv")
...
all_div2 <- merge(all_div, genome_size, by="bin")
 ...
 setwd("/data/projects/p898_Deer_RAS_metagenomics/04_Deer/LONG_READS/Diversity_all")
write.csv(all_div2, "LR28SR70_DEER_diversity.csv", row.names=F)
