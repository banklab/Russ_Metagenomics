...
for(i in 1:length(snp_file_list)){
  ...
  setwd("/data/projects/p898_Deer_RAS_metagenomics/04_Deer/Outliers_LR_ONLY/Diversity")
  write.csv(div_by_site, paste0(DEER,"_",ENV, "_LR28_Diversity_by_site.csv"), row.names = F)
  
  diversity_df_for_sample <- diversity_df[diversity_df$Deer==DEER & diversity_df$Env==ENV,]
  
  
  write.csv(diversity_df_for_sample, paste0(DEER,"_",ENV, "_LR28_Diversity_by_sample.csv"), row.names = F)
  
}


Diversity_files <- list.files(pattern="LR28_Diversity_by_sample.csv")
...
setwd("/data/projects/p898_Deer_RAS_metagenomics/04_Deer/Outliers_LR_ONLY/Diversity")
write.csv(all_div2, "LR28_DEER_diversity.csv", row.names=F)
