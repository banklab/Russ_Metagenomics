...
setwd("/data/projects/p898_Deer_RAS_metagenomics/04_Deer/REDO_SR_Binning/12_CMH/02_Filtered_Sites")
snp_list <- list.files(pattern="Filter_snps")
...
EnvA <- as.numeric(gsub(".*_Env|xEnv.*","",snp_list2[i]))
EnvB <- as.numeric(gsub(".*xEnv|_Filter.*","",snp_list2[i]))

if(EnvA==EnvB){stop("awklenew")}
