...
instrain_files <- list.files(pattern="Pooled")

EnvA_list <- unique(as.numeric(gsub("_Env.*","",gsub(".*Pooled_Env","",instrain_files))))


for(Good_Env in EnvA_list){

EnvB_list <- unique(as.numeric(gsub(".*_Env","",gsub("_snps.csv","",instrain_files[grepl(paste0("Pooled_Env",Good_Env),instrain_files)]))))
  
for(Miss_Env in EnvB_list){
