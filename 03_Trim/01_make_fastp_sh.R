...
for(i in 1:length(Read_list)){
  ...
  decompress1 <- paste0("gunzip -cd ",INDIR,"/",reads1," > ",OUTDIR,"/",sub("\\.gz","",reads1))
  ...
  write ("module load UHTS/Quality_control/fastp/0.19.5", sh_name, append = TRUE)
  write (decompress1, sh_name, append = TRUE)
  write (decompress2, sh_name, append = TRUE)
  write (fastp_code, sh_name, append = TRUE)
  write (paste0("gzip ",OUTDIR,"/",sub("fastq.gz","trim.fastq",reads1)), sh_name, append = TRUE)
