...
for(DEER in 1:7){
  ...
  setwd("/data/projects/p898_Deer_RAS_metagenomics/04_Deer/REDO_SR_Binning/08_InStrain/RAW")
 snv_file <- data.frame(fread(paste0(DEER,"_",ENV,"_SR_InStrain_SNVs.tsv"), header=T, stringsAsFactors = F))
  ...
  write.csv(snv_file2, paste0(DEER,"_",ENV,"_SR_InStrain_SNVs_format.csv"), row.names = F)
...
for(DEER in 1:7){
  ...
  snv_file3 <- data.frame(fread(paste0(DEER,"_",ENV,"_SR_InStrain_SNVs_format.csv"), header=T, stringsAsFactors = F))
  ...
  write.csv(full_df, paste0("ENV",ENV,"_SR_SNPS.csv"), row.names = F)
