...
cmh.function <- function(one.site){
  ...
  site.df <- cmh_input[cmh_input$Sp.ID==one.site,]
   ...
   if(length(unique(site.df$Env))<2){return(NULL)}

  number.deer <- table(site.df$Deer)
  
  site.df <- site.df[site.df$Deer %in% names(number.deer[number.deer>1]),]
...
cmh_list <- list.files(pattern=(paste0("_Env",EnvA,"xEnv",EnvB,"_drep")))
