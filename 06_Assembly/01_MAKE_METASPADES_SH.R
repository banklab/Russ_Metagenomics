...
for(i in 1:length(sample_list)){ ## loop over each sample, write a new sh script for each sample
  ...
  code_block <- paste0("metaspades.py --pe1-1 ",INPUT_DIR,"/",samp2,".R1.dedup.fastq --pe1-2 ",INPUT_DIR,"/",samp2,".R2.dedup.fastq --meta -o ",OUTPUT_DIR,"/",samp2,"_deer.asm -t 16 -m 195 -k 21,33,55,77")
