...
diversity_df <- data.frame(fread("DEER_v2_Abundance.csv", header=T, stringsAsFactors = F))
...
for(i in 1:length(snp_file_list)){
 ...
 snp_DF <- data.frame(fread(snp_file_list[i], header=T, stringsAsFactors = F))
  ...
  species_list <- species_list[!grepl("SR_",species_list)]
  
  DEER <- as.numeric(gsub("_.*","",snp_file_list[i]))
  ENV <- as.numeric(gsub(".*_","",gsub("_LR_InStrain.*","",snp_file_list[i])))
  ...
  for(j in 1:length(species_list)){
    ...
    genome_H <- sum(single_pop_df$H) / unique(diversity_df[diversity_df$bin==species_list[j],"genome.size"])
    ...
    diversity_df[diversity_df$bin==species_list[j] & diversity_df$Deer==DEER & diversity_df$Env==ENV,"polymorphic"] <- sum(diallelic, multiallelic) / unique(diversity_df[diversity_df$bin==species_list[j],"genome.size"])
    ...
    ## pi within
    genome_pi <- sum(single_pop_df$pi) / unique(diversity_df[diversity_df$bin==species_list[j],"genome.size"])
