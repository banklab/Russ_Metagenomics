...
for(i in 1:length(Read_list)){
  ...
  code_block <- paste0("bowtie2 -p 20 -x ",ASM_DIR,"/",reads1,"_",DATASET,".asm/scaffolds_filtered -1 ",READ_DIR,"/",reads1,"_R1.dedup.fastq -2 ",READ_DIR,"/",reads1,"_R2.dedup.fastq -S ",OUTPUT_DIR,"/",reads1,".sam")
