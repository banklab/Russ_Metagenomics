...
python consolidate_two_sets_of_bins.py $DIR/03_Temp_Winner/"$i"_deer/round1 $DIR/01_Bins/CONCOCT/"$i"_deer_concoct $DIR/03_Temp_Winner/"$i"_deer/round1.stats $DIR/02_Stats/"$i"_CONCOCT_stats.txt $DIR/03_Temp_Winner/"$i"_deer/round2 70 10
...
cp -r $DIR/03_Temp_Winner/"$i"_deer/round6/*fa $DIR/04_Top_Bins/"$i"_deer ## Copy winning bins from all comparisons into Top_Bins directory
cp $DIR/03_Temp_Winner/"$i"_deer/round6.stats $DIR/04_Top_Bins/STATS/"$i".stats ## Copy stats file from winning bins
